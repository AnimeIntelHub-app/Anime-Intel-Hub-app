...
async function testConnection(retries = 3) {
  ...
  try {
    ...
    if (retries > 0) {
      console.warn(`Firebase connection pending... retrying (${retries} left)`);
    ...
    if (error instanceof Error && (error.message.includes('the client is offline') || error.message.includes('deadline-exceeded'))) {
      console.warn("Firestore backend unreachable. This may be a temporary network issue in the preview environment. The app will continue in offline mode.");
    } else {
